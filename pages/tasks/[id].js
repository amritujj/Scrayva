...
import { 
  ...
  Loader2, FileJson, Table2, ArrowLeft, Clock, Activity, Database
} from 'lucide-react';
...
export default function TaskDetail() {
  ...
  const fetchTask = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // FIX: Safe JSON Parsing (agar result string mein save ho gaya tha)
        if (typeof data.result === 'string') {
          try { data.result = JSON.parse(data.result); } 
          catch(e) { /* It's a plain string, let it be */ }
        }
        setTask(data);
      } else {
        setTask(null);
  ...
  if (loading) {
    return (
      <div className="bg-black text-white min-h-screen flex flex-col font-sans selection:bg-[#0ea5e9]/30">
        <div className="hidden md:block"><Sidebar /></div>
        <div className="flex-1 ml-0 md:ml-64 flex flex-col items-center justify-center gap-4">
          <Loader2 className="w-8 h-8 animate-spin text-[#0ea5e9]" />
          <p className="text-slate-500 text-sm animate-pulse tracking-widest uppercase">Establishing Connection...</p>
        </div>
      </div>
    );
  }
  ...
  if (!task) {
    return (
      <div className="bg-black text-white min-h-screen flex flex-col font-sans selection:bg-[#0ea5e9]/30">
        <div className="hidden md:block"><Sidebar /></div>
        <div className="flex-1 ml-0 md:ml-64 flex items-center justify-center flex-col gap-4">
          <XSquare className="w-12 h-12 text-red-500/50 mb-2" />
          <p className="text-red-400 font-mono font-bold tracking-widest uppercase">ERR_TASK_NOT_FOUND</p>
          <p className="text-slate-500 text-sm">The task may have been deleted or never existed.</p>
  ...
  return (
    <div className="bg-black text-slate-200 min-h-screen flex flex-col font-sans selection:bg-[#0ea5e9]/30 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col ml-0 md:ml-64 h-screen">
        {/* ─── Top Control Bar ─── */}
        <header className="border-b border-white/10 bg-black/80 backdrop-blur-xl px-6 py-4 flex items-center justify-between z-40">
          <div className="flex items-center gap-6">
            <Link href="/dashboard" className="p-2 hover:bg-white/10 rounded-xl transition-colors text-slate-400 hover:text-white border border-transparent hover:border-white/10">
        ...
        <main className="flex-1 flex flex-col lg:flex-row h-full overflow-hidden">
          ...
          {/* ─── Left Panel: Metadata ─── */}
          <aside className="w-full lg:w-[350px] border-b lg:border-b-0 lg:border-r border-white/5 bg-[#09090b] flex flex-col overflow-y-auto">
            <div className="p-6">
                  ...
                  {task.screenshot ? (
                    ...
                    <div className="flex flex-col items-center gap-6">
                      <div className="relative w-24 h-24 flex items-center justify-center">
                        ...
                        <Terminal className="w-8 h-8 text-[#0ea5e9]" />
                      </div>
